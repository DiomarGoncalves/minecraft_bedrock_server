...
import { Send, Terminal, Trash2, ArrowDownCircle, PauseCircle } from 'lucide-react';
...
const Console: React.FC<CommonProps> = ({ showToast }) => {
  ...
  const sendCommand = (e: React.FormEvent) => {
    ...
    if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
        showToast('error', 'Console disconnected', 'Cannot send command.');
        return;
    }
    ...
    <div className="flex flex-col h-full bg-gray-950 rounded-xl overflow-hidden border border-gray-800 shadow-2xl relative">
      <div className="bg-gray-900 px-4 py-2 border-b border-gray-800 flex justify-between items-center">
        <div className="flex items-center space-x-2 text-gray-400">
          <Terminal size={18} />
          <span className="text-sm font-mono hidden sm:inline">bedrock_server</span>
        </div>
        <div className="flex items-center space-x-2">
            <button onClick={() => setAutoScroll(!autoScroll)} className={`p-1.5 rounded hover:bg-gray-700 transition-colors ${autoScroll ? 'text-emerald-400' : 'text-gray-500'}`}>
                {autoScroll ? <ArrowDownCircle size={18} /> : <PauseCircle size={18} />}
            </button>
            <button onClick={() => setLogs([])} className="p-1.5 rounded hover:bg-gray-700 text-gray-400 hover:text-red-400">
                <Trash2 size={18} />
            </button>
            <div className={`w-2 h-2 rounded-full mx-2 ${connected ? 'bg-emerald-500' : 'bg-red-500'}`} />
        </div>
      </div>
      
      <div ref={logContainerRef} className="flex-1 p-4 overflow-y-auto font-mono text-xs sm:text-sm bg-black text-gray-300">
        {logs.map((log, idx) => (
          <div key={idx} className={`${getLogColor(log.type, log.message)} whitespace-pre-wrap break-all leading-snug`}>
            {log.message}
          </div>
        ))}
      </div>
      ...
      <form onSubmit={sendCommand} className="bg-gray-900 p-2 flex items-center border-t border-gray-800">
        <div className="flex-1 relative">
            ...
            className="w-full bg-gray-800 text-white pl-8 pr-4 py-2.5 rounded-lg focus:outline-none focus:ring-1 focus:ring-emerald-500 font-mono text-sm"
            disabled={!connected}
            />
        </div>
        <button type="submit" disabled={!connected} className="ml-2 p-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-500 disabled:opacity-50">
            <Send size={18} />
        </button>
      </form>
    </div>
